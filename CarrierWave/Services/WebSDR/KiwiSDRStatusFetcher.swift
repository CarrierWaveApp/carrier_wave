...
/// Provides richer metadata than the public directory: SNR, grid, antenna details.
actor KiwiSDRStatusFetcher {
    ...
    static let shared = KiwiSDRStatusFetcher()

    /// Fetch /status for multiple receivers concurrently (max 5 in flight).
    /// Results stream back as they arrive.
    func fetchStatuses(
            ...
            Task {
                await withTaskGroup(of: ReceiverStatus?.self) { group in
                    ...
                    while index < receivers.count {
                        // Limit concurrency to maxConcurrent
    ...
    /// Parse key=value plain text response from /status
    private func parseStatusResponse(
        ...
        let antConnected: Bool = if let val = fields["ant_connected"] {
            ...
            true // Assume connected if field missing
        }
