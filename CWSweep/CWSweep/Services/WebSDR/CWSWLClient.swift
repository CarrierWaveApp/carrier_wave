...
nonisolated private let log = Logger(
    subsystem: "com.jsvana.CWSweep", category: "CW-SWL"
)
...
enum CWSWLError: Error, LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .noServerConfigured:
            "No cw-swl server configured. Set the URL in Settings > SDR."
        case .invalidURL:
        ...
        case .serverUnreachable:
            "Cannot reach cw-swl server. Check the URL in Settings > SDR."
        }
    }
}
...
actor CWSWLClient {
    ...
    func uploadRecording(
        ...
        log.info("[CW-SWL] Uploading \(body.count) bytes to \(url)")
    ...
    /// Convenience: upload + transcribe + poll until complete (5 min timeout).
    func transcribe(
        fileURL: URL,
...
private struct TranscriptionRequest: Encodable, Sendable {
...
private struct TranscriptionJobResponse: Decodable, Sendable {
    let id: UUID
}

// MARK: - TranscriptionStatusResponse

private struct TranscriptionStatusResponse: Decodable, Sendable {
    enum CodingKeys: String, CodingKey {
        case status
        case progress
        case transcript
