...
enum ContinentMapper {
    ...
    static func continent(forDXCC number: Int) -> String? {
        // North America
        if northAmerica.contains(number) { return "NA" }
        // South America
        if southAmerica.contains(number) { return "SA" }
        // Europe
        if europe.contains(number) { return "EU" }
        // Africa
        if africa.contains(number) { return "AF" }
        // Asia
        if asia.contains(number) { return "AS" }
        // Oceania
        if oceania.contains(number) { return "OC" }
        return nil
    }
