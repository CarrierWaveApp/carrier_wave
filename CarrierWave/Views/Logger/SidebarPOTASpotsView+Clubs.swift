...
extension SidebarPOTASpotsView {
    ...
    var nonClubSpotsByBand: [(band: String, spots: [POTASpot])] {
        groupSpotsByBand(nonClubPOTASpots)
    }

    var nonClubSOTAByBand: [(band: String, spots: [SOTASpot])] {
        groupSOTASpotsByBand(nonClubSOTASpots)
    }
        ...
        ForEach(nonClubSpotsByBand, id: \.band) { section in
            Section {
                ForEach(section.spots) { spot in
                    ...
                    .contextMenu { sidebarTuneInMenu(for: spot) }
                    Divider()
                        .padding(.leading, 92)
                }
            } header: {
    ...
    func sidebarTuneInMenu(for spot: POTASpot) -> some View {
        Button {
