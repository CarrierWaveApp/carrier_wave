...
extension BragSheetComputationActor {
    func computeFurthestContact(
        ...
        guard let best = snapshots.compactMap({ qso -> (BragSheetQSOSnapshot, Double)? in
            guard let km = qso.distanceKm else { return nil }
            return (qso, km)
        }).max(by: { $0.1 < $1.1 }) else {
            return .noData
        }
...
extension BragSheetComputationActor {
    ...
    func computeBestWattsPerMile(
        ...
        let candidates = snapshots.compactMap { qso
            -> (BragSheetQSOSnapshot, Double, Double)? in
            guard let power = qso.power, power > 0,
                  let km = qso.distanceKm, km > 0
            else { return nil }
            let miles = km * 0.621371
            let wpm = Double(power) / miles
            return (qso, wpm, miles)
        }
        ...
        let detail = String(
            ...
            best.0.power ?? 0,
            best.0.callsign,
            UnitFormatter.distance(best.0.distanceKm ?? 0),
            best.0.band
        )
        return .efficiency(wattsPerMile: best.1, detail: detail)
    }
}
...
extension BragSheetComputationActor {
    ...
    func computeNewDXCC(
        ...
        for dxcc in periodDXCC {
            guard let all = allByDXCC[dxcc] else { continue }
    ...
    func computeContinents(
        ...
        guard !continents.isEmpty else { return .noData }
    ...
    func computeMostContinentsDay(
        ...
        guard best > 0 else { return .noData }
        return .count(best)
    }
