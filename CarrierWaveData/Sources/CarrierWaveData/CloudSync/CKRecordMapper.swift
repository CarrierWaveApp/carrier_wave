...
public enum CKRecordMapper {
    ...
    nonisolated public static let zoneName = "CarrierWaveData"
    nonisolated public static let zoneID = CKRecordZone.ID(
        zoneName: zoneName, ownerName: CKCurrentUserDefaultName
    )

    nonisolated public static let metadataModes: Set<String> = ["WEATHER", "SOLAR", "NOTE"]

    // MARK: - Record Identity

    /// Build a deterministic CKRecord name from entity type and UUID
    nonisolated public static func recordName(entityType: String, id: UUID) -> String {
        "\(entityType)-\(id.uuidString)"
    }

    nonisolated public static func recordID(type: RecordType, id: UUID) -> CKRecord.ID {
        let name = recordName(entityType: type.rawValue, id: id)
        return CKRecord.ID(recordName: name, zoneID: zoneID)
    }

    /// Parse a UUID from a CKRecord name.
    /// Supports both formats:
    /// - New: "EntityType-UUID" (e.g., "QSO-A1B2C3D4-E5F6-...")
    /// - Legacy: raw UUID (e.g., "A1B2C3D4-E5F6-...")
    nonisolated public static func parseUUID(from recordName: String) -> UUID? {
        // Try legacy format first: raw UUID
        if let uuid = UUID(uuidString: recordName) {
            return uuid
        }
        // New format: "EntityType-UUID"
    ...
    /// Returns nil for legacy records with raw UUID names.
    nonisolated public static func parseEntityType(from recordName: String) -> String? {
        // Legacy format: raw UUID — no entity type prefix
        if UUID(uuidString: recordName) != nil {
            return nil
        }
        guard let dashIndex = recordName.firstIndex(of: "-") else {
            return nil
    ...
    nonisolated public static func shouldSync(mode: String) -> Bool {
        !metadataModes.contains(mode.uppercased())
    }

    /// Convert QSOFields to a CKRecord
    nonisolated public static func qsoFieldsToCKRecord(
        _ fields: QSOFields,
    ...
    nonisolated public static func qsoFields(from record: CKRecord) -> QSOFields? {
    ...
    nonisolated public static func servicePresenceFieldsToCKRecord(
    ...
    nonisolated public static func servicePresenceFields(
