...
private extension AzimuthalMapView {
    ...
    func drawDistanceRings(context: GraphicsContext, center: CGPoint, radius: CGFloat) {
        ...
        for ringKm in Self.ringDistancesKm where ringKm < maxDistanceKm {
            let r = CGFloat(ringKm / maxDistanceKm) * radius
            ...
            context.draw(
                ...
                at: CGPoint(x: center.x + r + 2, y: center.y),
                anchor: .leading
            )
        }
    }
    ...
    func drawAntennaPattern(
        ...
        for (index, point) in points.enumerated() {
            ...
            let r = CGFloat(point.radius) * patternRadius
            let x = center.x + r * CGFloat(sin(radians))
            let y = center.y - r * CGFloat(cos(radians))
