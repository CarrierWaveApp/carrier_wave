...
enum BragSheetStatValue: Sendable, Equatable {
...
struct BandTableEntry: Sendable, Equatable, Identifiable {
    let band: String
    let callsign: String
...
struct ModeStreakEntry: Sendable, Equatable, Identifiable {
    let mode: String
    let current: Int
    let longest: Int
...
extension BragSheetStatValue {
    ...
    var heroValue: String {
        ...
        case let .duration(s): formatDurationCompact(s)
        case let .rate(v, _): String(format: "%.0f", v)
        case let .contact(call, _, _): call
        case let .power(w, _, _, _): "\(w)W"
        case let .efficiency(wpm, _): String(format: "%.3f", wpm)
        case let .progress(c, t): "\(c)/\(t)"
        case let .streak(c, _): "\(c)d"
