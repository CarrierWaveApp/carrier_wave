...
struct BragSheetQSOSnapshot: Sendable {
    let id: UUID
    let callsign: String
    let band: String
    let mode: String
    ...
    var isQRP: Bool {
        guard let power else { return false }
        return power <= 5
    }

    /// Whether this is a milliwatt QSO (less than 1W).
    var isMilliwatt: Bool {
        guard let power else { return false }
        return power < 1
    }
...
extension BragSheetQSOSnapshot {
    init(from qso: QSO) {
        id = qso.id
